...
fn factorized_aggregate_gate(
    ...
    let (factors, nvars) = crate::leapfrog::parse_body_factors(&pat_expr)?;
    if factors.len() < 2 {
...
/// Without the feature there is no decomposition to consult, so the enumerate sink always runs.
#[cfg(not(feature = "factorized_aggregate"))]
fn factorized_aggregate_gate(
    _read_copy: &PathMap<()>,
    _pat_expr: Expr,
    _sinks: &[crate::sinks::ASink],
    _templates: &[Expr],
) -> Option<FactorizedAgg> {
    None
}
