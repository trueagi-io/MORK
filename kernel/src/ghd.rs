...
//!
//! The WCO leapfrog join (`leapfrog`) runs a whole conjunction as one global
...
use crate::leapfrog::{parse_body_factors, run_unify_join_stream, Factor, FactorColumn};
use mork_expr::{unify, Bindings, Expr, ExprEnv};
use pathmap::PathMap;
...
/// The query variables one factor constrains. `EncodedTerm` already carries the mask its parse
/// scan filled in, so a `Term` column is read rather than re-walked.
fn collect_factor_vars(factor: &Factor, out: &mut BTreeSet<usize>) {
    for col in &factor.cols {
        match col {
            FactorColumn::Var(v) => {
                out.insert(*v);
...
/// Byte length of the first complete subterm at `bytes[0..]`, from the encoding's own walk.
fn first_subterm_len(bytes: &[u8]) -> usize {
    unsafe { Expr::from_slice(bytes).span().as_ref().unwrap().len() }
}

/// Whether a column is a compound carrying variables, which the bag key would not see.
fn is_nonground_compound(col: &FactorColumn<'_>) -> bool {
    matches!(col, FactorColumn::Term(term) if !term.is_ground())
}

/// GHD drop-in for `leapfrog::query_multi_leapfrog`: for a CYCLIC body with a low-width hypertree
...
/// answer set is byte-identical. Returns `None` -- fall back to the global join -- for an acyclic
/// body (already output-optimal), a body with a nonground compound column (a shared variable the
/// bag key would miss), or when no width `<= 3` decomposition exists.
pub fn query_multi_ghd<F: FnMut(Result<&[u32], &Bindings>, Expr) -> bool>(
    map: &PathMap<()>,
    pat_expr: Expr,
    mut effect: F,
) -> Option<usize> {
    ...
    // Every variable must be a top-level column: `extract_var_values` reads top-level columns, so
    // a nonground compound column could hide a shared variable the bag-join key misses.
    if factors.iter().any(|f| f.cols.iter().any(is_nonground_compound)) {
        return None;
    }
    let edges = hypergraph(&factors);
    let ghd = decompose(&edges, 3)?;
    ...
    let bags_mat: Vec<(Bag, Vec<BagMatch>)> = ghd
        .bags
        .iter()
        .map(|b| (b.clone(), materialize_bag(map, &factors, b, nvars)))
        .collect();
    let full_tuples = join_bags(&bags_mat, factors.len());
    ...
    for tuple in &full_tuples {
        let e = Expr { ptr: tuple[0].as_ptr().cast_mut() };
        let mut pairs = vec![(sources[0], ExprEnv::new(1, e))];
        for (j, fact) in tuple.iter().enumerate().skip(1) {
            pairs.push((
                sources[j],
                ExprEnv::new((j + 1) as u8, Expr { ptr: fact.as_ptr().cast_mut() }),
            ));
        }
